...
def build_teaching_visualization_prompt(
    *,
    ...
    return truncate_text(
        f"""
You are generating an interactive HTML teaching artifact for 知进拍学.
...
async def observe_demo_extract(
    ...
    不调用 save_upload、不写任何用户表。默认要求登录；若确需公开演示，可显式设置 PXJ_OBSERVE_DEMO_PUBLIC=true。"""
    init_db()
...
def _render_restore_html(qset: list[dict], view: str, base_url: str) -> str:
    ...
    for i, q in enumerate(qset):
        ...
        if is_blank:
            ...
            if thumb_src:
                caption = "题目裁剪" if crop_src else "原图核对"
    ...
    if analysis_image_rows:
        ...
        emit_log(
            (
                ...
                f"题目裁剪 {crop_saved_count} 张（重复 {crop_duplicate_count}、跳过 {crop_skipped_count}）；"
        ...
        if invalid_count:
            emit_log(
                f"收到智能观察批次：{len(filenames)} 张，其中无相关画面 {invalid_count} 张，题目裁剪 {crop_saved_count} 张，已跳过大模型解析",
                session_id=session_id,
                device_id=device_id,
            )
        else:
            emit_log(f"收到重复观察批次：{len(filenames)} 张，题目裁剪 {crop_saved_count} 张，无新增关键画面，已跳过大模型解析", session_id=session_id, device_id=device_id)
    return {
        "session_id": session_id,
        "batch_id": batch_id,
        "analysis_id": analysis_id,
