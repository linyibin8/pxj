...
- 本项目后续默认“开发完成即同步发布”：涉及后端、iOS 或前后端联动的有效代码改动，完成验证后必须继续部署生产后端并发布 iOS TestFlight。
- 除非明确说明“只本地修改”“先别发布”或“只做方案/评审”，否则不要停在本地实现。
- 发布顺序默认是：后端编译/冒烟 -> 生产后端部署 -> iOS generic build -> TestFlight 发布 -> 线上/真机验收。
- 若发布被阻塞，必须明确说明阻塞原因、当前已完成的验证、下一步需要的操作。
